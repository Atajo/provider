...
module.exports = {
    ...
    build: function(processId, version, cb) {
        var _ = this;



        if (processId > 0) { cb(true); return; }
        ...
        */
        ...
        var sassDir = path.join(baseVersionDir, 'scss');
        var lessDir = path.join(baseVersionDir, 'less');
        ...
        var sassFiles = _.getFiles(sassDir, 'scss', '_', false); //atajo.log.d("HTMLFILES --> "+JSON.stringify(htmlFiles));
        // var lessFiles = _.getFiles(lessDir, 'scss', false, false); //atajo.log.d("HTMLFILES --> "+JSON.stringify(htmlFiles));
        ...
        //COMPILE SASS
        _.compileSass(sassFiles, path.join(cssDir, 'scss.css'), function() {
    ...
    packageFiles: function(files, ext, type, vers, cb) {
        ...
        atajo.log.d("PACKAGING " + files.length + "FILES -> " + vers + " / " + ext + " / " + type);

        compressor.minify({
            compressor: type,
            input: files,
            output: path.join(__dirname, '../', 'build', '/base-' + vers + '.' + ext),
