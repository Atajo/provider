...
var builder = {
    ...
    processTS: function(version) {
        ...
        return new Promise((resolve, reject) => {
            ...
            for (var t in TSFiles) {
                ...
                var tsFileName = tsFile.split('/');
                tsFileName = tsFileName[tsFileName.length - 1];

                var tsContent = fs.readFileSync(tsFile, 'utf8');
                var tsPath = tsFile.split(_.COMPILE_DIR + '/')[1];
                var tsPathRemainder = tsPath.split(tsFileName)[0];


                //atajo.log.d("PATH REMAINDER IS : " + tsPathRemainder);
                ...
                for (var h in HTMLFiles) {
                    ...
                    // atajo.log.d("IS " + htmlPath + " IN " + tsFile + "?");
            ...
            //CREATE SHIM
            //_.generatePatch("<ion-app>pp</ion-app>", path.join(_.BUILD_PATH, 'base-' + version.BASEVERSION + '.html'));
            ...
            webpack.on('close', (code) => {
                ...
                if (code == 0) {
                    ...
                    fs.copySync(, path.join(_.BUILD_PATH, 'base-' + version.BASEVERSION + '.js'));
    ...
    processCSS: function(version) {
        ...
        return new Promise(function(resolve, reject) {
            ...
            atajo.log.d("BUILDING CSS");

            var cssBuildFile = path.join(_.BUILD_PATH, 'base-' + version.BASEVERSION + '.small.css');
            ...
            _.compileSass(SASSFiles, cssBuildFile, function() {

                atajo.log.d("SASS DONE");
                ...
                if (CSSContent == '') { CSSContent = '.noDirectCSS {}'; }

                fs.appendFileSync(cssBuildFile, CSSContent, 'utf8');
                atajo.log.d("CSS DONE");
                fs.copySync(cssBuildFile, path.join(_.BUILD_PATH, 'base-' + version.BASEVERSION + '.large.css'));
                resolve();
    ...
    compileSass: function(files, outFile, cb) {

        atajo.log.d("COMPILING SASS FILES : " + JSON.stringify(files) + " --> " + outFile);

        if (!outFile) {
            atajo.log.e("OUTFILE NOT SPECIFIED");
            cb();
            return;
        } else {
